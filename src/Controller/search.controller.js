...
try {
    let response;
    const {donorName,number} = req.query;
    if(!donorName){
        response = await DaanDarta.find({contactNo:number});
        return res.status(200).json(new ApiResponse(200,response,"List generated with Donor Number"))
    }else if(!number){
        response = await DaanDarta.find({donorName:donorName})
        return res.status(200).json(new ApiResponse(200,response,"List generated with Donor Name"))
    }else{
        response = await DaanDarta.find({$and:[{donorName:donorName,contactNo:number}]})
        return res.status(200).json(new ApiResponse(200,response,"List generated with Donor Name and Number"))
    }
...
async function SearchMilkVolume (req,res){
    ...
    try {
           ...
           return res.status(200).json(new ApiResponse(200,response,"Milk Volume generated by donor name"))
    } catch (error) {
